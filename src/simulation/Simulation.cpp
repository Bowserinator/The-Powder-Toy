...
void Simulation::init_can_move()
{
	...
	for (destinationType = 0; destinationType < PT_NUM; destinationType++)
	{
		...
		if (destinationType == PT_GLAS || destinationType == PT_PHOT || destinationType == PT_FILT || destinationType == PT_INVIS
		 ...
		 || destinationType == PT_GLOW || elements[destinationType].Properties & PROP_WATER
...
int Simulation::try_move(int i, int x, int y, int nx, int ny)
{
	...
	case PT_CNCT:
		...
		break;
	case PT_RCRT:
		if (y < ny && TYP(pmap[y+1][x]) == PT_RCRT) return 0;
		break;
	case PT_GBMB:
		if (parts[i].life > 0)
			return 0;
			...
			else if (elements[t].Properties & TYPE_ENERGY)
			{
				if (t == PT_PHOT)
				{
					...
					if (eval_move(PT_PHOT, fin_x, fin_y, NULL))
					{
						...
						int lt = TYP(pmap[y][x]);
						int rt_glas = (rt == PT_RDMD) || (rt == PT_GLAS) || (rt == PT_BGLA);
						int lt_glas = (rt == PT_RDMD) || (lt == PT_GLAS) || (lt == PT_BGLA);
